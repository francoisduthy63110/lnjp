    ...
    <div className="min-h-screen bg-white text-slate-900 p-6">
      <div className="max-w-xl mx-auto space-y-4">
        ...
        <div className="rounded-xl border p-4 space-y-3">
          <input className="w-full border rounded-lg p-2" placeholder="ADMIN_TOKEN (MVP)"
                 value={token} onChange={e => setToken(e.target.value)} />
          <input className="w-full border rounded-lg p-2" placeholder="Titre"
                 value={title} onChange={e => setTitle(e.target.value)} />
          <textarea className="w-full border rounded-lg p-2" placeholder="Message"
                    value={body} onChange={e => setBody(e.target.value)} />
          <input className="w-full border rounded-lg p-2" placeholder="URL (optionnel)"
                 value={url} onChange={e => setUrl(e.target.value)} />

          <button
            className="w-full rounded-xl bg-slate-900 text-white px-4 py-3 font-semibold"
            onClick={async () => {
              setResult(null);
          ...
          {items.map((n) => (
            <div key={n.id} className="border rounded-xl p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                {!n.readAt && (
                  <span className="text-xs bg-slate-900 text-white rounded-full px-2 py-1">Nouveau</span>
                )}
              </div>
              ...
              <div className="mt-3 flex gap-2">
                ...
                {n.url && (
                  <a className="text-sm rounded-lg border px-3 py-2" href={n.url}>
                    Ouvrir
                  </a>
                )}
              </div>
